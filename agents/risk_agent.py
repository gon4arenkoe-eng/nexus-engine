...
Проверяет сигналы перед исполнением, отслеживает лимиты.
"""
...
from models import Position, BotSettings, TradeHistory
from app import db
...
class RiskAgent(BaseAgent):
    ...
    - Max leverage
    - Daily loss limit
    - Position size vs balance
    - Duplicate signals (same symbol, same direction)
    """

    def __init__(self):
        super().__init__("risk")
        self._daily_pnl_cache: Dict[int, Decimal] = {}  # user_id -> daily PnL
    ...
    def run(self, signal: Dict[str, Any], user_id: int, 
        ...
        try:
            ...
            # Check 2: Duplicate position (same symbol, same side)
            for pos in current_positions:
                if pos["symbol"] == symbol and pos["side"] == ("LONG" if side == "BUY" else "SHORT"):
            ...
            # Calculate SL/TP from signal metadata or defaults
    ...
    def _get_daily_pnl(self, user_id: int) -> Decimal:
        ...
        total_pnl = sum((t.pnl or 0) + (t.commission or 0) + (t.funding_fee or 0) for t in trades)
    ...
    def reset_daily_pnl(self, user_id: int) -> None:
        """Reset daily PnL cache (call at midnight)."""
