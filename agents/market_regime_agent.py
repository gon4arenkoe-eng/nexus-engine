...
class MarketRegimeAgent(BaseAgent):
    ...
    def __init__(self, chop_period: int = 14, chop_threshold_trend: float = 50.0, chop_threshold_range: float = 61.8,
                 bb_period: int = 20, bb_std_dev: int = 2, kc_period: int = 20, kc_atr_multiplier: float = 1.5):
    ...
    async def run(self, symbol: str, data: pd.DataFrame) -> Dict[str, Any]:
        if not self._validate_data(data, min_rows=max(self.chop_period, self.bb_period, self.kc_period) + 2):
            logger.warning(f"MarketRegimeAgent: Insufficient data for {symbol}")
            self.bus.publish("market.regime", {"symbol": symbol, "regime": "NEUTRAL", "reason": "Insufficient data"})
            return {"symbol": symbol, "regime": "NEUTRAL", "reason": "Insufficient data"}

        df = data.copy()

        # Calculate indicators
        df["chop"] = Indicators.calculate_choppiness_index(df["high"], df["low"], df["close"], self.chop_period)
        df["bb_sma"], df["upper_bb"], df["lower_bb"] = Indicators.calculate_bollinger_bands(df["close"], self.bb_period, self.bb_std_dev)
        df["kc_ema"], df["upper_kc"], df["lower_kc"] = Indicators.calculate_keltner_channels(df["high"], df["low"], df["close"], self.kc_period, self.kc_atr_multiplier)
        ...
        if current["chop"] < self.chop_threshold_trend:
            regime = "TRENDING"
            reason = f"Choppiness Index ({current["chop"]:.2f}) indicates trending market."
        elif current["chop"] > self.chop_threshold_range:
            regime = "RANGING"
            reason = f"Choppiness Index ({current["chop"]:.2f}) indicates ranging market."
        else:
            ...
            if is_squeeze:
                ...
                reason = "Bollinger Bands are inside Keltner Channels (squeeze detected)."
            else:
                regime = "NEUTRAL"
