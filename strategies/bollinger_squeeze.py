...
class BollingerSqueezeStrategy(BaseStrategy):
    ...
    def __init__(self, bb_period: int = 20, bb_std_dev: int = 2, kc_period: int = 20, kc_atr_multiplier: float = 1.5):
    ...
    def analyze(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        if not self._validate_data(data, min_rows=max(self.bb_period, self.kc_period) + 2):
        ...
        if is_squeeze:
            ...
            if current["close"] > current["upper_bb"] and previous["close"] <= previous["upper_bb"]:
                signal = "BUY"
                confidence = 85 # Высокая уверенность в прорыве вверх
                reason += ", Price broke above Upper BB"
            elif current["close"] < current["lower_bb"] and previous["close"] >= previous["lower_bb"]:
                signal = "SELL"
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            ...
            "kc_atr_multiplier": {"value": self.kc_atr_multiplier, "type": "float", "min": 1.0, "max": 3.0},
        }
