...
class TrendFollowingChopStrategy(BaseStrategy):
    ...
    def __init__(self, fast_ema: int = 9, slow_ema: int = 21, rsi_period: int = 14, chop_period: int = 14, chop_threshold_trend: float = 50.0):
    ...
    def analyze(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        if not self._validate_data(data, min_rows=max(self.slow_ema, self.rsi_period, self.chop_period) + 2):
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            ...
            "chop_period": {"value": self.chop_period, "type": "int", "min": 7, "max": 30},
            "chop_threshold_trend": {"value": self.chop_threshold_trend, "type": "float", "min": 30.0, "max": 70.0},
        }
