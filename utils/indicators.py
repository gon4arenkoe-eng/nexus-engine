...
class Indicators:
    ...
    def calculate_choppiness_index(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14) -> pd.Series:
        ...
        denominator = denominator.replace(0, np.nan) # Replace 0 with NaN to avoid division by zero
    ...
    @staticmethod
    def calculate_keltner_channels(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 20, atr_multiplier: float = 1.5) -> tuple[pd.Series, pd.Series, pd.Series]:
