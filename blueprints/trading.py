...
def get_positions():
    ...
    positions = asyncio.run(trading_service.position_agent.run(request.current_user.id, exchange.id, None))
...
def get_pnl():
    ...
    return jsonify({"daily_pnl": float(pnl) if pnl else 0, "positions_count": len(positions)})
...
def update_settings():
    ...
    allowed = ["symbols", "timeframe", "confidence_threshold", "max_positions",
