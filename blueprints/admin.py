...
def reset_db():
    ...
    if data.get("confirmation") != "RESET_ALL_DATA_CONFIRM":
        return jsonify({"error": "Confirmation required", "message": "Send confirmation: 'RESET_ALL_DATA_CONFIRM'"}), 400
    db.drop_all()
    db.create_all()
    return jsonify({"message": "Database reset successfully", "warning": "All data has been deleted"})
